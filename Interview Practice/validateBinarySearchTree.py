...
class Solution:
    def isValidBST(self, root: TreeNode) -> bool:
        ...
        return self.validate(root.left, float('-inf'), root.val - 1) and self.validate(root.right, root.val + 1, float('inf'))
    ...
    def validate(self, node, minval, maxval):
        ...
        if node.val > maxval or node.val < minval:
            ...
            return self.validate(node.right, node.val + 1, maxval) and self.validate(node.left, minval, node.val - 1)
